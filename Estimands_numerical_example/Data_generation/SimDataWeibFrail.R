...
SimDataWeibFrail = function(n.sample, params, coeff_A=NULL, 
                            no.protected = T, no.large = T,
                            cens.exp.rate = 0.1, cens, cens.admin = 100, 
                            round.times = T, X = NULL, dim_x, tau){
    ...
    for (i in 1:n.sample.temp) {
      ...
      if (T2.1[i] >= T1.1[i]) {
        ...
        if (round.times == T) {
          ...
          if (T2.1[i] == T1.1[i]) {
            T2.1[i] <- round(T1.1[i] + runif(1, 0.1, 1), 1)
          }
        }
      }
    }
  ...
  T1[A == 0] <- pmin(T1.0[A == 0], T2.0[A == 0], 1)
  T1[A == 1] <- pmin(T1.1[A == 1], T2.1[A == 1], 1)
  T2[A == 0] <- pmin(T2.0[A == 0], 1)
  T2[A == 1] <- pmin(T2.1[A == 1], 1)
  delta1[A == 0] <- as.integer(T1[A == 0] == T1.0[A == 0])
  delta1[A == 1] <- as.integer(T1[A == 1] == T1.1[A == 1])
  #delta2 <- ifelse(A==1, 1-S_1, 1-S_0)
  delta2[A == 0] <- as.integer(T2[A == 0] == T2.0[A == 0])
  delta2[A == 1] <- as.integer(T2[A == 1] == T2.1[A == 1])
...
# CausalSemiComp::SimDataWeibFrail
# shape = k (alpha), scale = 1/lambda
SimDataWeibFrail_criterion = function(n.sample, params, coeff_A=NULL
                            ,no.protected = T, no.large = T # NO need in no.protected and no.large
                            ,cens.exp.rate = 0.1, cens, cens.admin = 100
                            ,round.times = T, X = NULL, dim_x, tau, criterion="none"){
  list2env(params, envir = environment())
  gamma.scale <- params$theta
  gamma.common.shape <- params$rho / params$theta
  gamma.each.shape <- (1 - params$rho) / params$theta
  ...
  while (cond.sample == F) {
    ...
    if (iter_count > 20) stop("criterion filtering removes too many observations")
    
    if (is.null(X_input)) { # if (is.null(x)) 
      x1 <- rbinom(n = n.sample.temp, size = 1, prob = 0.5)
      x2 <- rnorm(n.sample.temp)
      if(dim_x==2){X <- cbind(x1, x2)}
      if(dim_x==1){X <- x2}
      ...
      n.times <- ceiling(n.sample.temp/nrow(X_input)) # nrow(X_input)
      X <- do.call(rbind, replicate(n.times, X_input, simplify = F)) %>% as.matrix # do.call(rbind, replicate(n.times, X, simplify = F))
      n.sample.temp <- nrow(X)
    }
    ...
    if (criterion == "ios_ORP") {
      out <- ((T1.0 < tau) | (T2.0 >= tau)) & !((T1.1 < tau) | (T2.1 >= tau))
    } else if (criterion == "ORP") {
      out <- (T1.0 < tau) & !(T1.1 < tau)
    } else if (criterion == "weak_ORP") {
      out <- (T1.0 < tau) & !((T1.1 < tau) | (T2.1 >= tau))
    } else {
      out <- rep(FALSE, length(T1.0))
    }
